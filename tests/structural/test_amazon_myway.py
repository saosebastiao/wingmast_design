...
def test_partition_is_manufacturable_for_3_and_4_spars():
    ...
    spec = AmazonMastSpec()
    for n in (3, 4):
        fit = myway_fit(spec, MyWayParams(n_spars=n))
...
def test_closer_webs_lower_the_buckling_floor_wall():
    ...
    spec = AmazonMastSpec()
    # at the thin masthead region the buckling floor governs; the cell wall must shrink with n_spars
    z = 0.8 * spec.sail_track_length
    _, sec3, _ = myway_section(z, spec, MyWayParams(n_spars=3))
    _, sec4, _ = myway_section(z, spec, MyWayParams(n_spars=4))
...
def test_myway_beats_sponberg_at_first_order():
    ...
    feasible = []
    for n in (3, 4):
        r = estimate_myway_mass(spec, MyWayParams(n_spars=n), amazon_per_mast_kg=amz.mass_per_mast_kg)
        if r.feasible:
            feasible.append(r)
    ...
    r3 = estimate_myway_mass(spec, MyWayParams(n_spars=3))
    r4 = estimate_myway_mass(spec, MyWayParams(n_spars=4))
    assert r3.mass_per_mast_kg < r4.mass_per_mast_kg


def test_mass_bookkeeping():
    spec = AmazonMastSpec()
    r = estimate_myway_mass(spec, MyWayParams(n_spars=3))
