...
def _params(x: np.ndarray, n_cells: int, base: MyWayParams) -> MyWayParams:
    f0 = float(x[5])
    off = 0.5 * (1.0 - f0)                                    # balance ±45 / 90 in the cap
    return replace(base, n_cells=n_cells, box_frac_chord=float(x[0]), box_frac_thick=float(x[1]),
                   t_shell=float(x[2]), t_web=float(x[3]), shell_helix_deg=float(x[4]),
                   layup_f0=f0, layup_f45=off, layup_f90=off)


def _objective(x: np.ndarray, spec: AmazonMastSpec, n_cells: int, base: MyWayParams,
    ...
    # (n_cells, walls). It is verified ONCE on the optimum in `optimize_myway`. This keeps each
    # eval cheap. Strength + cap-buckling are met by the inner sizer; deflection is an OPTIONAL cap.
    p = _params(x, n_cells, base)
...
def optimize_myway(spec: AmazonMastSpec, n_cells: int, amazon_per_mast_kg: float,
...
def optimize_amazon_beat(spec: AmazonMastSpec | None = None, amazon_per_mast_kg: float = 646.0,
                         *, cell_counts=(3, 4, 5), seed: int = 0, defl_cap_m: float | None = None,
    ...
    runs = [optimize_myway(spec, n, amazon_per_mast_kg, seed=seed, defl_cap_m=defl_cap_m,
                           maxiter=maxiter, popsize=popsize, workers=workers) for n in cell_counts]
